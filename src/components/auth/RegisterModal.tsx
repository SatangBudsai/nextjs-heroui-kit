...
export default function RegisterModal({ isOpen, onClose, onOpenLogin }: RegisterModalProps) {
  ...
  const onSubmit: SubmitHandler<RegisterFormData> = async data => {
    try {
      ...
      if (response.data && response.data.user && response.data.token) {
        ...
        Alert.error({
          content: 'เกิดข้อผิดพลาดในการสมัครสมาชิก'
        })
      }
    } catch (error: any) {
      if (error?.response?.data?.message === 'Email already exists') {
        Alert.error({
          content: 'มี Email นี้ในระบบแล้ว กรุณาใช้ Email อื่น'
        })
      } else {
        Alert.error({
          content: 'เกิดข้อผิดพลาดในการสมัครสมาชิก'
  ...
  useEffect(() => {
    ...
    if (isSubmitted && (password || confirmPassword)) {
      trigger('confirmPassword')
    }
  }, [watch('password'), watch('confirmPassword'), isSubmitted])
      ...
      <ModalContent>
        <ModalBody className='px-8 pb-8'>
          <section className='mb-2 flex flex-col items-center px-8 pt-8'>
            ...
            <p className='text-center text-sm text-gray-600'>สร้างบัญชีใหม่เพื่อเข้าใช้งาน Onlearn Engenius</p>
          </section>
                  ...
                  <Input
                    ...
                    startContent={
                      <Icon
                        ...
                        width={18}
                        height={18}
                        className={!!errors.name ? 'text-danger' : 'text-primary'}
                      />
                    }
